...
fun eval_prog (e,env) =
    case e of
      ...
      | Shift (dx, dy, exp) => case eval_prog (exp, env) of
				 ...
				 | _ => raise BadProgram "Shift applied to non-value"
					      

(* takes a geom_exp and checks all LineSegments. If its endpoints are 
the same, it returns a new Point. It returns a new LineSegment with 
switched endpoints if x2 > x1 or x2 and x1 are the same but y2 > y1.
Else it returns the same LineSegment *)
